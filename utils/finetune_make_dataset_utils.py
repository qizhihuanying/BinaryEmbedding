...
def get_positives(df: pd.DataFrame, pct_size: float) -> pd.DataFrame:
    ...
    # 给每个生成的文本加上进度条
    sample1 = list(tqdm(sample_from_text(df["original"].tolist(), pct_size), desc="Generating sample1 (positives)", unit="pair"))
    sample2 = list(tqdm(sample_from_text(df["original"].tolist(), pct_size), desc="Generating sample2 (positives)", unit="pair"))
...
def get_negatives(df: pd.DataFrame, pct_size: float) -> pd.DataFrame:
    ...
    # 给每个生成的文本加上进度条
    sample1 = list(tqdm(sample_from_text(df["original"].tolist(), pct_size), desc="Generating sample1 (negatives)", unit="pair"))
    sample2 = list(tqdm(circular_rotate(sample_from_text(df["original"].tolist(), pct_size), 1), desc="Generating sample2 (negatives)", unit="pair"))
